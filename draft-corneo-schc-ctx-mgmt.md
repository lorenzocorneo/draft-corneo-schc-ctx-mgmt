...
multi-layer protocol compression without combinatorial rule explosion.
...
SCHC was initially designed for low-power wide-area networks serving a
...
- Rule referencing CDAs ({{rule-referencing}}) that allow rules to
  reference and compose other rules, reducing duplication.

- A rule fragment branching mechanism ({{rule-fragment-branching}})
  that enables dynamic selection of rule fragments based on packet
  content, eliminating combinatorial explosion.
...
: A set of field descriptions within one rule that belong to a single
  protocol layer.

Deprecated Rule:
: A rule (or subset of a rule) that has been selected to be merged
  into a new rule using referencing CDAs.

Merged Rule:
: A rule that includes at least one of the referencing CDAs defined
  in this document.

Rule Fragment:
: A rule designed to compress a portion of a packet (e.g., a single
  protocol layer) and intended to be composed with other rule
  fragments via branching.

# Rule Referencing {#rule-referencing}

This section defines two novel Compression/Decompression Actions (CDAs)
...
When ref(N) is used, the Target Value (TV) and Field Length (FL) fields
in the referencing field description are not used for compression.
Instead, the TVs and FLs of the referenced Rule N apply.
...
## The ref-edit(N,M) CDA

The "ref-edit" CDA extends ref(N) by allowing modification of specific
...
immediately following the ref-edit field that specify modifications.
...
2. For each of the next M field descriptions in the current rule,
   find the field description in Rule N with a matching Field ID (FID).
...
1. Iterate all rules in the context. For each rule, identify sets of
   adjacent field descriptions (field descriptions belonging to the
   same protocol layer).

2. Group identical sets of adjacent field descriptions that appear in
   multiple rules.
...
# Rule Fragment Branching {#rule-fragment-branching}
...
multi-layer protocol stacks or protocols with optional/variable headers.
...
no further rule processing follows.
...
- Explicit field matching (match-mapping MO, see {{match-mapping-mo}}):
...
- Implicit matching (match-rule MO, see {{match-rule-mo}}): Used when
...
Specifically, the compressor maintains a branch queue per rule. After
...
The compressor MUST transmit the packet uncompressed (using Rule ID 0).

A branch to NULL terminates further processing, even if there are
queued invocations in parent rules.

As a consequence, when the packet content following a branch point may
be unknown or not covered by any rule fragment, the branch mapping
SHOULD include a branch(NULL) entry as a fallback. Without it, an
...
uncompressed in the SCHC residual.

## The match-mapping MO with branch {#match-mapping-mo}

When branch is used with a non-zero FL, the existing match-mapping MO
(defined in {{RFC8724}}) is used to determine which branch to take. The
...
entry selects the corresponding branch argument.
...
{: #tab-nh-mapping title="Next Header Branch Mapping"}
...
## The match-rule MO {#match-rule-mo}
...
compression is applied to the payload.

## Interaction Between branch and ref(N)

The branch CDA and the ref(N) CDA (defined in {{rule-referencing}})
...
## IPv6/UDP/TCP Rule Composition Using ref(N) and ref-edit(N,M)

This example shows how ref(N) and ref-edit(N,M) eliminate duplication
when multiple transport protocols share the same IPv6 compression rule.
...
referencing CDAs, a single IPv6 rule (Rule 2 in {{fig-rule2}}) is
defined once and referenced by the UDP rule (Rule 3 in {{fig-rule3}})
using ref(2), and the TCP rule (Rule 4 in {{fig-rule4}}) using
ref-edit(2,1) to override the Next Header value from 17 to 6.

For a context with 3 IPv6 address variants and 4 transport/application
...
1. The compressor iterates through the rule set and attempts to match
   each rule against the packet. Rule 2 ({{fig-branch-ipv6}}) is
...
3. After completing Rule 2, the compressor loads Rule 3
   ({{fig-ex-udp}}). UDP fields are matched and compressed.
...
5. Rule 8 ({{fig-ex-rtp}}) compresses the RTP header fields.
...
- Maximum reference chain depth (number of nested ref/ref-edit/branch
  invocations).
- Maximum number of entries in a branch mapping table.
- Maximum number of candidate rules evaluated by match-rule.
- Total processing time for a single packet compression/decompression
  operation.
...
mechanisms MUST ensure integrity and authenticity of rule definitions.
